document.addEventListener('DOMContentLoaded', () => {
    ...
    // Fail-safe safety fallback timeout (1.5s) to guarantee visibility
    setTimeout(() => {
        fadeSections.forEach(section => {
    ...
    /* --- 11. PROJECTS COLLAPSIBILITY --- */
    const projectsToggleBtn = document.getElementById('projectsToggleBtn');
    const collapsibleProjects = document.getElementById('collapsibleProjects');

    if (projectsToggleBtn && collapsibleProjects) {
        projectsToggleBtn.addEventListener('click', () => {
            const isExpanded = collapsibleProjects.classList.contains('expanded');
            
            if (isExpanded) {
                // Collapse projects container
                collapsibleProjects.style.maxHeight = collapsibleProjects.scrollHeight + 'px';
                // Force a reflow
                collapsibleProjects.offsetHeight;
                
                collapsibleProjects.style.maxHeight = '0px';
                collapsibleProjects.style.opacity = '0';
                collapsibleProjects.style.transform = 'translateY(30px)';
                collapsibleProjects.classList.remove('expanded');
                
                projectsToggleBtn.setAttribute('aria-expanded', 'false');
                projectsToggleBtn.querySelector('.btn-text').textContent = 'Explore More Analytics Projects';
                projectsToggleBtn.querySelector('.btn-icon').textContent = '▼';

                // Scroll the page smoothly back to the start of projects section
                const projectsSection = document.getElementById('projects');
                if (projectsSection) {
                    projectsSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
                }
            } else {
                // Expand projects container
                collapsibleProjects.style.maxHeight = collapsibleProjects.scrollHeight + 'px';
                collapsibleProjects.style.opacity = '1';
                collapsibleProjects.style.transform = 'translateY(0)';
                collapsibleProjects.classList.add('expanded');
                
                projectsToggleBtn.setAttribute('aria-expanded', 'true');
                projectsToggleBtn.querySelector('.btn-text').textContent = 'Hide Additional Projects';
                projectsToggleBtn.querySelector('.btn-icon').textContent = '▲';

                // Allow accordions to expand freely without parent clipping once the transition finishes
                collapsibleProjects.addEventListener('transitionend', function handler(e) {
                    if (e.propertyName === 'max-height' && collapsibleProjects.classList.contains('expanded')) {
                        collapsibleProjects.style.maxHeight = 'none';
                        collapsibleProjects.removeEventListener('transitionend', handler);
                    }
                });
            }
        });
